...
Releases are fully automated with [semantic-release](https://semantic-release.gitbook.io/). You do not bump the version, create the tag, or write the changelog by hand.

Every push to `main` runs `.github/workflows/release.yml`, which runs `semantic-release`. It computes the next version from the commit types, publishes to npm, creates the GitHub Release with the autogenerated release notes (that is where the changelog lives now — one entry per release), attaches the standalone binaries + `SHA256SUMS` + `install.sh`, and creates the `vX.Y.Z` tag. Version bumps follow the commit types: `fix:`/`perf:` → patch, `feat:` → minor, `feat!:` or a `BREAKING CHANGE:` footer → major. Commits of type `docs`, `chore`, `ci`, `test`, `style`, `refactor` do not trigger a release on their own.

npm publishing uses **OIDC Trusted Publishing** (no `NPM_TOKEN`): the `release` job has `id-token: write`, forces npm ≥ 11.5.1, and npm generates provenance automatically. `@semantic-release/npm` runs `npm publish` in that trusted context. Trusted publishing is configured once on npmjs.com (package → Settings → Trusted publishing): organization `rafaelvieiras`, repository `pocketstack-cli`, workflow `release.yml`.

semantic-release does **not** commit or push anything back to `main` — no changelog commit, no version bump commit. That is deliberate: because nothing is pushed back, `main` can carry strict branch protection (required PRs, no direct pushes) without breaking the automation. The repo's `package.json` therefore keeps `version: 0.0.0-development` on purpose; the real version of each release lives in the git tags and on npm, not in the tree. The published artifact still reports the correct version because `@semantic-release/npm` sets the version in `package.json` before the build/publish step runs, so the bundled `VERSION` is baked from the bumped value.

To preview a release without publishing, trigger the workflow manually (Actions → Release → Run workflow) with **dry run** enabled.

The self-updater (`pocketstack upgrade`) checks the npm registry for the latest version and upgrades in place: binaries re-run `install.sh`; npm installs re-run `npm install -g`.
